...
namespace Proto
{
    ...
    public class BiomeDresser : MonoBehaviour
    {
        ...
        /// </summary>
        /// <summary>
        /// Menukar SELURUH daftar wajah — dipakai saat run berpindah TEMPAT (act baru: hutan ->
        /// salju -> ternoda). Indeks wajah dipertahankan, bukan direset ke siang: pemain yang
        /// menyeberang act di tengah malam harus tiba di malam tempat berikutnya, karena yang
        /// berganti tempatnya, bukan jamnya.
        /// </summary>
        public void UseFaces(BiomeDefinition[] faces)
        {
            if (faces == null || faces.Length == 0) return;

            int idx = Mathf.Clamp(FaceIndex, 0, faces.Length - 1);
            _biomes = faces;
            Apply(faces[idx]);
        }
        ...
        void Apply(BiomeDefinition biome)
        {
            ...
            // — batang, tajuk, dan pohon mesh memudar di sekitar pemain lewat
            // Grimoire/PropSeeThrough. Rumput & batu TIDAK ikut: sependek itu ia tidak pernah
            // menghalangi, dan memudarkannya cuma menambah biaya antrean transparan.
        ...
        /// </summary>
        // Lapisan asli terrain + salinan runtime-nya. Salinan dibuat MALAS: run yang tidak
        // pernah meninggalkan hutan tidak membayar apa pun.
        TerrainLayer[] _originalLayers;
        TerrainData _runtimeData;

        /// <summary>
        /// Menukar TerrainLayer terrain mengikuti biome — inilah yang membuat tanah bersalju.
        ///
        /// Peta catnya TIDAK disentuh. Splatmap menyimpan BOBOT per lapisan, terpisah dari
        /// daftar lapisannya, jadi menukar daftar dengan jumlah yang sama berarti pola yang
        /// dicat tangan tetap utuh — bercak matahari tetap bercak, jalur tanah tetap jalur —
        /// hanya catnya yang berganti dari rumput ke salju.
        ///
        /// Ditukar pada SALINAN runtime TerrainData, bukan pada asetnya. TerrainData adalah
        /// aset; mengubahnya saat play mode di editor MENETAP setelah play berhenti, dan lantai
        /// hutan yang diam-diam tinggal bersalju adalah cara paling halus merusak scene tanpa
        /// satu pun error. Salinannya dibuat sekali saat pertama kali dibutuhkan, lalu dipakai
        /// terus — termasuk untuk kembali ke lapisan asli.
        /// </summary>
        void ApplyGroundLayers(BiomeDefinition biome)
        {
            if (_terrain == null || _terrain.terrainData == null) return;

            bool custom = biome.GroundLayers != null && biome.GroundLayers.Length > 0;

            // Belum pernah menukar dan biome ini tidak minta apa-apa: jalur lama, nol biaya.
            if (!custom && _runtimeData == null) return;

            if (_runtimeData == null)
            {
                _originalLayers = _terrain.terrainData.terrainLayers;

                _runtimeData = Instantiate(_terrain.terrainData);
                _runtimeData.name = _terrain.terrainData.name + " (runtime)";

                _terrain.terrainData = _runtimeData;

                var collider = _terrain.GetComponent<TerrainCollider>();
                if (collider != null) collider.terrainData = _runtimeData;
            }

            _runtimeData.terrainLayers = custom ? biome.GroundLayers : _originalLayers;
        }
