...
namespace Proto
{
        ...
        ISelectHandler, IDeselectHandler
    {
        ...
        [Tooltip("Latar tombol (anak 'Hit') yang ikut meredup saat baris tidak disorot. " +
                 "KOSONG = tidak ada yang disentuh — baris menu polos tetap polos. Permintaan " +
                 "pemilik project: banner tombol starter jangan pekat terus-terusan.")]
        [SerializeField] UnityEngine.UI.Image _backdrop;

        [Tooltip("Alpha latar saat diam. Disorot = naik ke 1.")]
        [Range(0f, 1f)] [SerializeField] float _backdropIdle = 0.35f;
        ...
        void ApplyBlend()
        {
            ...
            // TANPA sprite, latar tidak pernah digambar — Image kosong yang dinaikkan
            // alphanya cuma jadi kotak putih polos (pemilik project sengaja mengosongkan
            // sprite Hit starter: "mau transparan atau hide"). Raycast tetap jalan di
            // alpha nol, jadi tombolnya tetap bisa diklik.
            if (_backdrop != null)
            {
                var c = _backdrop.color;
                c.a = _backdrop.sprite == null ? 0f : Mathf.Lerp(_backdropIdle, 1f, _blend);
                _backdrop.color = c;
            }
        }
