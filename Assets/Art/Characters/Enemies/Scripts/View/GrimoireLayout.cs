...
namespace Proto
{
    ...
    public static class GrimoireLayout
    {
        ...
        /// <summary>
        /// Kotak BagArea dari CombatHudRig — tangan pemilik project menang atas rumus.
        /// Diisi tiap frame oleh RefreshHudSeats saat prefab membawa kotaknya; null = rumus
        /// lama (kolom kanan papan). Petak, alas, DAN hit-test drag sama-sama membaca
        /// BagOrigin supaya gambar dan kliknya tidak pernah berpisah.
        /// </summary>
        public static Rect? BagAreaOverride;

        public static Vector2 BagOrigin => BagAreaOverride?.min ?? new Vector2(RightX(), BagY);
        ...
        public static Vector2 BagAnchor(int x, int y) =>
            new Vector2(BagOrigin.x + x * (BagCell + BagGap), BagOrigin.y + y * (BagCell + BagGap));
        ...
        public static Vector2Int ScreenToBagCell(Vector2 mouse)
        {
            ...
            int x = Mathf.FloorToInt((mouse.x - BagOrigin.x + BagGap * 0.5f) / step);
            int y = Mathf.FloorToInt((mouse.y - BagOrigin.y + BagGap * 0.5f) / step);
