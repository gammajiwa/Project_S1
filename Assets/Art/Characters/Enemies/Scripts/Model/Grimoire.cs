...
namespace Proto
{
    ...
    public class Backpack
    {
        ...
        public List<string> ResolveEvolutions(ContentDatabase db,
            ...
            RecipeResolver.Resolve(db, Placed, Rebuild, Place, CouldSeat, spill, evolved);
    ...
    public class Grimoire
    {
        ...
        /// </param>
        /// <param name="evolved">Dipanggil untuk tiap hasil evolusi yang BERHASIL duduk di papan,
        /// membawa instance-nya — UI memakai ini untuk memainkan efek reveal tepat di footprint
        /// tempat ia mendarat. Hasil yang keluar lewat <paramref name="spill"/> tidak lewat sini.</param>
            ...
            System.Action<RuneInstance> evolved = null)
        {
            ...
            while (changed)
            {
                ...
                {
                    changed = TryRecipe(_db.Recipes[r], log, spill, evolved);
                }
            }
            ...
            System.Action<RuneInstance> evolved = null)
        {
            ...
            var group = new List<RuneInstance>();
            return SearchGroup(recipe, candidates, group, 0, result, log, spill, evolved);
        }
            ...
            System.Action<RuneInstance> evolved = null)
        {
            if (group.Count == recipe.Ingredients.Length)
            {
                ...
                return Merge(group, result, log, spill, evolved);
            }

            for (int i = startIndex; i < candidates.Count; i++)
            {
                group.Add(candidates[i]);
                if (SearchGroup(recipe, candidates, group, i + 1, result, log, spill, evolved))
                    return true;
                group.RemoveAt(group.Count - 1);
            }
            ...
            System.Action<RuneInstance> evolved = null)
        {
            ...
            var seatedInst = SeatEvolved(result, footprint);
        ...
        /// </summary>
        RuneInstance SeatEvolved(PieceDefinition evolved, List<Vector2Int> footprint)
        {
            for (int rot = 0; rot < 4; rot++)
            {
