...
namespace Proto.EditorTools
{
    ...
    public static class VatBaker
    {
        ...
        /// </param>
        /// <param name="roleByName">
        /// Pemetaan NAMA KLIP ke peran, dipasang tangan. Kosong = tebak dari namanya seperti biasa.
        ///
        /// Perlu ada karena tebakan nama tidak punya kosakata untuk TERBANG. Naga membawa
        /// <c>Walk</c>, <c>Run</c>, dan <c>Fly Forward</c> sekaligus; tebakannya akan memilih dua
        /// yang pertama lalu memanggang naga yang berjalan kaki, sementara klip terbangnya —
        /// satu-satunya alasan model itu dipakai — tidak ikut masuk tekstur sama sekali.
        ///
        /// Yang disebut di sini juga menentukan klip MANA yang dipanggang, bukan cuma perannya.
        /// </param>
        public static VatClipSet Bake(GameObject source, string clipFolder = null,
            Dictionary<string, VatRole> roleByName = null)
        {
            ...
            else
            {
                ...
                clips = roleByName != null && roleByName.Count > 0
                    ? PickByName(clips, roleByName)
                    : PickOnePerRole(clips);
            }
            ...
            var result = BakeRig(source, rig, smr, clips, folder, roleByName);
            Object.DestroyImmediate(rig);
            return result;
        }

        static VatClipSet BakeRig(GameObject source, GameObject rig, SkinnedMeshRenderer smr,
            List<AnimationClip> clips, string folder, Dictionary<string, VatRole> roleByName)
        {
            ...
            for (int i = 0; i < clips.Count; i++)
            {
                ...
                {
                    Role = RoleFor(clips[i].name, roleByName),
        ...
        /// <summary>
        /// Peran klip ini: yang dipasang tangan kalau ada, tebakan nama kalau tidak.
        /// </summary>
        static VatRole RoleFor(string name, Dictionary<string, VatRole> roleByName)
        {
            VatRole role;
            if (roleByName != null && roleByName.TryGetValue(name, out role)) return role;
            return RoleOf(name);
        }

        /// <summary>
        /// Klip yang disebut namanya, satu per peran, diurutkan menurut perannya.
        ///
        /// Yang tidak ketemu dilaporkan lalu dilewati, bukan diganti diam-diam: peran yang hilang
        /// jatuh ke klip pertama saat main (lihat <see cref="VatClipSet.TryGet"/>), dan diamnya
        /// kesalahan itu jauh lebih mahal daripada satu baris peringatan saat memanggang.
        /// </summary>
        static List<AnimationClip> PickByName(List<AnimationClip> pool,
            Dictionary<string, VatRole> roleByName)
        {
            var picked = new List<AnimationClip>(roleByName.Count);

            foreach (var kv in roleByName)
            {
                AnimationClip found = null;

                for (int i = 0; i < pool.Count; i++)
                {
                    if (pool[i].name != kv.Key) continue;
                    found = pool[i];
                    break;
                }
                ...
                if (found == null)
                {
                    Debug.LogWarning("[VatBaker] klip '" + kv.Key + "' untuk peran " + kv.Value +
                                     " tidak ketemu — peran itu tidak ikut dipanggang.");
                    continue;
                }
            ...
            picked.Sort((a, b) =>
                RoleFor(a.name, roleByName).CompareTo(RoleFor(b.name, roleByName)));
