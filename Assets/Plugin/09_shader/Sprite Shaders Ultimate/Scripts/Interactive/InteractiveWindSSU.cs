...
namespace SpriteShadersUltimate
{
    ...
    public class InteractiveWindSSU : InstancerSSU
    {
        ...
        [Tooltip(
            "The editor-side script set's the layer to 'Ignore Raycast' to fix potential issues. Enable this to disable that and set the layer to a different one.")]
        public bool allowCustomLayer = false;

        //Variables:
        HashSet<Collider> collidersInside;
        HashSet<Collider2D> collidersInside2D; // Added 2D set
        BoxCollider boxCollider;
        BoxCollider2D boxCollider2D; // Added 2D Collider
        ...
        void Start()
        {
            ...
            collidersInside2D = new HashSet<Collider2D>(); // Init 2D set
            boxCollider = GetComponent<BoxCollider>();
            boxCollider2D = GetComponent<BoxCollider2D>(); // Get 2D Collider
        ...
        void FixedUpdate()
        {
                ...
                if (collider != null)
                {
                    ...
                    else
                    {
                        if (!newDirection)
                        {
                            ...
                            if ((currentRotationDirection < 0 && newLocalPosition.x > localPosition.x) ||
                                (currentRotationDirection > 0 && newLocalPosition.x < localPosition.x))
                            {
                        ...
                        {
                            localPosition = ((Vector2)(collider.bounds.center - transform.position) + localPosition) *
                                            0.5f; //Position Deviation (multiple colliders)
                        }
                    }
                }
            }

            // Process 2D Colliders
            foreach (Collider2D collider in collidersInside2D)
            {
                if (collider != null)
                {
                    if (localPosition.y < -99999)
            ...
            if (localPosition.y > -99999) //Colliders are interacting with the wind sprite.
            {
                ...
                // Get Width based on available collider
                float boundsSizeX = 1f;
                if (boxCollider != null) boundsSizeX = boxCollider.size.x;
                else if (boxCollider2D != null) boundsSizeX = boxCollider2D.size.x;

                //Bend Target:
                float targetBending = 0;
                if (currentRotationDirection < 0)
                {
                    targetBending = Mathf.Clamp01((localPosition.x + boundsSizeX * 0.5f) / boundsSizeX);
                }
                else
                {
                    targetBending = Mathf.Clamp01((boundsSizeX * 0.5f - localPosition.x) / boundsSizeX);
                }
                ...
                //Fade In Bending:
                currentBending += (currentBendTarget * currentRotationDirection - currentBending) *
                                  Mathf.Min(bendInSpeed * Time.fixedDeltaTime, 1);
                UpdateShader();
            }
            else
            {
        ...
        {
            // Debug.Log($"[InteractiveWind] 3D Trigger Enter: {collision.gameObject.name}");
            if (collidersInside.Count == 0 && collidersInside2D.Count == 0 || Mathf.Abs(currentBending) < 0.2f)
            {
                newDirection = true;
            }
        ...
        {
            // Debug.Log($"[InteractiveWind] 2D Trigger Enter: {collision.gameObject.name}");
            if (collidersInside.Count == 0 && collidersInside2D.Count == 0 || Mathf.Abs(currentBending) < 0.2f)
            {
                newDirection = true;
            }

            collidersInside2D.Add(collision);
        ...
        {
            // Debug.Log($"[InteractiveWind] 2D Trigger Exit: {collision.gameObject.name}");
            if (collidersInside2D.Contains(collision))
            {
                collidersInside2D.Remove(collision);
            }
        }

        //Other:
        public static void DefaultCollider(BoxCollider box)
        {
            box.isTrigger = true;
